...
namespace avocado
{
	namespace backend
	{
		TEST(TestOptimizer, float32_sgd)
		{
			...
			data.set(AVOCADO_OPTIMIZER_SGD, 100, 0.01, { 0.0, 0., 0.0, 0.0 }, { false, false, false, false });
		...
		TEST(TestOptimizer, float32_sgd_momentum)
		{
			if (not supportsType(AVOCADO_DTYPE_FLOAT32))
				GTEST_SKIP();
			OptimizerTester data( { 23, 45 }, AVOCADO_DTYPE_FLOAT32);
			data.set(AVOCADO_OPTIMIZER_SGD, 100, 0.01, { 0.01, 0.0, 0.0, 0.0 }, { true, false, false, false });
			float alpha = 1.1f, beta = 0.1f;
			EXPECT_LT(data.getDifference(&alpha, &beta), 1.0e-4);
		}
		TEST(TestOptimizer, float32_sgd_nesterov)
		{
			if (not supportsType(AVOCADO_DTYPE_FLOAT32))
				GTEST_SKIP();
			OptimizerTester data( { 23, 45 }, AVOCADO_DTYPE_FLOAT32);
			data.set(AVOCADO_OPTIMIZER_SGD, 100, 0.01, { 0.01, 0.0, 0.0, 0.0 }, { true, true, false, false });
			float alpha = 1.1f, beta = 0.1f;
			EXPECT_LT(data.getDifference(&alpha, &beta), 1.0e-4);
		}
		TEST(TestOptimizer, float32_adam)
		{
			if (not supportsType(AVOCADO_DTYPE_FLOAT32))
				GTEST_SKIP();
			OptimizerTester data( { 23, 45 }, AVOCADO_DTYPE_FLOAT32);
			data.set(AVOCADO_OPTIMIZER_ADAM, 100, 0.01, { 0.01, 0.001, 0.0, 0.0 }, { false, false, false, false });
			float alpha = 1.1f, beta = 0.1f;
			EXPECT_LT(data.getDifference(&alpha, &beta), 1.0e-4);
		}
